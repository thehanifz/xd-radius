...
<div id="{{ $id }}"
    ...
    <div class="relative z-10 w-full max-w-sm mx-4 bg-white rounded-2xl shadow-2xl p-6 space-y-4 animate-modal-in">
        ...
        <div class="w-12 h-12 rounded-full bg-red-50 flex items-center justify-center mx-auto">
            <svg class="text-red-500" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <form method="POST" action="{{ $actionUrl }}" id="{{ $id }}-form">
            ...
            <label class="flex items-center gap-2.5 text-sm text-slate-500 select-none cursor-pointer">
                <input type="checkbox" id="{{ $id }}-pref" class="rounded"
                       onchange="setConfirmPref('{{ $preferenceKey }}', this.checked)">
...
@push('scripts')
<script>
const _confirmPrefs = JSON.parse(localStorage.getItem('confirm_prefs') || '{}');

function setConfirmPref(key, value) {
    _confirmPrefs[key] = value;
    localStorage.setItem('confirm_prefs', JSON.stringify(_confirmPrefs));
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('[data-modal-open]').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            // Cek preference — kalau user sudah pilih "jangan tanyakan lagi" langsung submit
            const prefKey = modal.querySelector('[id$="-pref"]')?.id.replace(id + '-', '').replace('-pref','') ?? null;
            const form    = document.getElementById(id + '-form');

            // Cari preference key dari form dataset
            const pk = modal.dataset.preferenceKey;
            if (pk && _confirmPrefs[pk]) {
                form && form.submit();
                return;
            }
...
.animate-modal-in { animation: modal-in 200ms cubic-bezier(0.34, 1.56, 0.64, 1) forwards; }
</style>
@endpush
@endonce
