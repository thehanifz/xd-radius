                        ...
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2 justify-end">
                                ...
                                <form method="POST" action="{{ route('members.toggle-status', $member) }}">
                                    @csrf @method('PATCH')
                                    <button type="submit"
                                        title="{{ $member->status === 'active' ? 'Isolir' : 'Aktifkan' }}"
                                        class="{{ $member->status === 'active' ? 'text-slate-400 hover:text-red-500' : 'text-slate-400 hover:text-green-600' }}">
                                ...
                                <form method="POST" action="{{ route('members.destroy', $member) }}"
                                    onsubmit="return confirm('Hapus member {{ $member->username }}?')">
                                    @csrf @method('DELETE')
